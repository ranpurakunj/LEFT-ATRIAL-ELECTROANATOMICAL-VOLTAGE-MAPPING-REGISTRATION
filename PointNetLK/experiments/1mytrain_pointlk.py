...
eval_name_count = 1
epoch_name_count = 1
myepochList = [ 1,2,50,100,150,200,250,300,350,400,450,500 ]
...
def run(args, trainset, testset, action):
    ...
    global epoch_name_count
    for epoch in range(args.start_epoch, args.epochs):
        #scheduler.step()
        myepoch = epoch_name_count
        running_loss, running_info = action.train_1(model, trainloader, optimizer, args.device,myepoch)
        val_loss, val_info = action.eval_1(model, testloader, args.device,myepoch)
        epoch_name_count += 1
...
class Action:
    ...
    def train_1(self, model, trainloader, optimizer, device,myepoch):
        model.train()
        vloss = 0.0
        gloss = 0.0
        count = 0
        global train_name_count
        for i, data in enumerate(trainloader):
            name_pre = "_train_"+ str(train_name_count)
            fun ="train"
            loss, loss_g = self.compute_loss(model, data, device,name_pre,myepoch,fun)
            train_name_count += 1
    ...
    def eval_1(self, model, testloader, device,myepoch):
        model.eval()
        vloss = 0.0
        gloss = 0.0
        count = 0
        global eval_name_count
        with torch.no_grad():
            for i, data in enumerate(testloader):
                name_pre =eval_name_count
                name_pre = "_eval_"+ str(eval_name_count)
                fun = "eval"
                loss, loss_g = self.compute_loss(model, data, device,name_pre,myepoch,fun)
                eval_name_count += 1
    ...
    def compute_loss(self, model, data, device,name_pre,myepoch,fun):
        global myepochList
        p0, p1, igt = data
        p0 = p0.to(device) # template
        p1 = p1.to(device) # source
        igt = igt.to(device) # igt: p0 -> p1
        if (((myepoch in myepochList) ==True) and (fun == "eval")):
        
          #Iteration-5
          r = ptlk.pointlk.PointLK.do_forward(model, p0, p1, 5, self.xtol,self.p0_zero_mean, self.p1_zero_mean).clone()
          #r = model(p0, p1, self.max_iter)
          est_g = model.g
          x = "Epoch"+str(myepoch)+"_"+name_pre +"_Iter5_"
          self.plot_pointcloud(est_g,p0,p1,x)
          #Iteration-10
          r = ptlk.pointlk.PointLK.do_forward(model, p0, p1, 10, self.xtol,self.p0_zero_mean, self.p1_zero_mean).clone()
          #r = model(p0, p1, self.max_iter)
          est_g = model.g
          x = "Epoch"+str(myepoch)+"_"+name_pre +"_Iter10_"
          self.plot_pointcloud(est_g,p0,p1,x)
          #Iteration-15
          r = ptlk.pointlk.PointLK.do_forward(model, p0, p1, 15, self.xtol,self.p0_zero_mean, self.p1_zero_mean).clone()
          #r = model(p0, p1, self.max_iter)
          x = "Epoch"+str(myepoch)+"_"+name_pre +"_Iter15_"
          est_g = model.g
          self.plot_pointcloud(est_g,p0,p1,x)
        
        #Iteration-20
        r = ptlk.pointlk.PointLK.do_forward(model, p0, p1, self.max_iter, self.xtol,self.p0_zero_mean, self.p1_zero_mean).clone()
        #r = model(p0, p1, self.max_iter)
        est_g = model.g
        if (((myepoch in myepochList) ==True) and (fun == "eval")):
        
          x = "Epoch"+str(myepoch)+"_"+name_pre +"_Iter20_"
          self.plot_pointcloud(est_g,p0,p1,x)
    ...
    def plot_pointcloud(self, est_g,p0,p1,name_pre):
        template, source, est_T = p0[0], p1[0], est_g[0]
        x = torch.zeros(len(source)).unsqueeze(1).to(source)
        source = torch.cat((source , x), dim =-1)
        transformed_source = torch.matmul(source,est_T)
        template =template.detach().numpy()
        transformed_source = transformed_source.detach().numpy()       
        fig = plt.figure(figsize=(8,8))
        ax = fig.add_subplot(111,projection='3d')
        ax.scatter(template[:,0],template[:,1],template[:,2],c ='b')
        ax.scatter(transformed_source[:,0],transformed_source[:,1],transformed_source[:,2],c ='r')
        plt.savefig('drive/MyDrive/PointNetLK-master/testresults/images/Test'+name_pre+'.jpg')
